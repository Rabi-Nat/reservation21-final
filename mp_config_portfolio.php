...
define('IMAGE_DIR', 'Images/Portfolio/');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    $file  = $_FILES['portfolio_pic'];
    $title = test_input($_POST['portfolio_title']) ?? "";

    // بررسی فایل آپلود
    if (!isset($file) || $file['error'] === UPLOAD_ERR_NO_FILE) {
        $_SESSION['salon_sample_file_error'] = "فایلی انتخاب نشده است.";
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }

    if ($file['error'] !== UPLOAD_ERR_OK) {
        $_SESSION['salon_sample_file_error'] = "خطا در آپلود فایل.";
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }

    if ($file['size'] > MAX_FILE_SIZE) {
        $_SESSION['salon_sample_fileSize_error'] = " حداکثر حجم مجاز فایل" . MAX_FILE_SIZE . " مگابایت می باشد";
        //mysqli_close($conn);
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }
    ...
    if (!in_array($mime, $allowed)) {
        $_SESSION['salon_sample_fileType_error'] = "فقط فایل‌های jpeg, png, gif, tiff, webp مجاز می‌باشند.";
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }

    // بررسی تعداد عکسهای موجود در دیتابیس
    $SqlCount = "SELECT COUNT(sample_id) AS cnt FROM salon_sample 
                    WHERE salon_id = ? AND photo_url <> '' ";
    ...
    if ($cnt >= MAX_FILE_NUM) {
        $_SESSION['salon_sample_num_error'] = "حداکثر می توانید " . MAX_FILE_NUM . " نمونه کار آپلود کنید";
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }
    ...
    if (!move_uploaded_file($file['tmp_name'], $fullPath)) {
        $_SESSION['salon_sample_file_error'] = "خطا در ذخیره فایل روی سرور.";
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }

    // درج در دیتابیس
    $insertSql = "INSERT INTO salon_sample (salon_id, photo_url, salon_photo_title)
                  VALUES (?,?,?)";
    $stmtInsert = mysqli_prepare($conn, $insertSql);
    mysqli_stmt_bind_param($stmtInsert, "iss", $salon_id, $dbPath, $title);

    if (mysqli_stmt_execute($stmtInsert)) {
        $_SESSION['mp_upload_sample'] = "نمونه کار با موفقیت ذخیره شد.";
    } else {
        // در صورت خطا، فایل را حذف کن
        @unlink($fullPath);
        $_SESSION['mp_upload_sample_error'] = "خطا در ذخیره‌سازی اطلاعات .";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_portfolio'])) {
    ...
    if (!is_array($sample_ids) || count($sample_ids) === 0) {
        $_SESSION['mp_delete_sample_error'] = "درخواست حذف نامعتبر است.";
        header("Location: manager_profile.php#portfolioPics");
        exit();
    }

    // آماده‌سازی statement: ابتدا select برای گرفتن مسیر فایل، سپس delete
    $qSelect = "SELECT photo_url FROM salon_sample WHERE sample_id = ? AND salon_id = ?";
    $qDelete = "DELETE FROM salon_sample WHERE sample_id = ? AND salon_id = ?";
    $stmtSelect = mysqli_prepare($conn, $qSelect);
    $stmtDelete = mysqli_prepare($conn, $qDelete);

    foreach ($sample_ids as $sid) {
        $sample_id = intval($sid);
        if ($sample_id <= 0) continue;

        // گرفتن مسیر فایل
        mysqli_stmt_bind_param($stmtSelect, 'ii', $sample_id, $salon_id);
        mysqli_stmt_execute($stmtSelect);
        mysqli_stmt_bind_result($stmtSelect, $photo_url);
        ...
        if (!empty($photo_url)) {
            $possiblePath = __DIR__ . '/' . $photo_url;
        ...
        // حذف رکورد
        mysqli_stmt_bind_param($stmtDelete, 'ii', $sample_id, $salon_id);
        if (mysqli_stmt_execute($stmtDelete)) {
            $_SESSION['mp_delete_sample'] = "نمونه کار با موفقیت حذف شد.";
        } else {
            $_SESSION['mp_not_delete_sample'] = "خطا در حذف نمونه کار.";
        }
    }
