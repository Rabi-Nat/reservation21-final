...
define('FILE_DIR', 'Files/customer_tickets/');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if (isset($file) && $file['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if (!in_array($mime, $allowed)) {
            $_SESSION['cp_ticket_fileType_error'] = "فقط فایل‌های jpeg, png, gif, tiff, webp, zip, rar, pdf, doc مجاز می‌باشند.";
            header("Location: customer_tickets.php");
            exit();
        }
        ...
        if (!move_uploaded_file($file['tmp_name'], $fullPath)) {
            $_SESSION['cp_ticket_file_error'] = "خطا در ذخیره فایل روی سرور";
            header("Location: customer_tickets.php");
            exit();
        }
        ...
        $query = "INSERT INTO customer_ticket (customer_id, ticket_title, ticket_category, ticket_message, ticket_file, ticket_file_orig)
                VALUES (?,?,?,?,?,?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'isssss', $customer_id, $ticketTitle, $ticketCategory, $ticketMessage, $newName, $originalName);
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['cp_upload_ticket'] = " تیکت با موفقیت ارسال شد";
        } else {
            // در صورت خطا، فایل را حذف کن
            @unlink($fullPath);
            $_SESSION['cp_upload_ticket_error'] = "خطا در ارسال تیکت ";
        }
    } else {
        // === without file ===
        // بدون فایل — ستون ticket_file_orig NULL می‌ماند
        $query = "INSERT INTO customer_ticket (customer_id, ticket_title, ticket_category, ticket_message)
                VALUES (?,?,?,?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'isss', $customer_id, $ticketTitle, $ticketCategory, $ticketMessage);
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['cp_upload_ticket'] = " تیکت با موفقیت ارسال شد";
        } else {
            // در صورت خطا، فایل را حذف کن
